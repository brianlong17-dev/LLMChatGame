...
from tests.helpers.game_test_helpers import QueuedClient, TestGameSink, TestSimulation, attach_test_runtime, make_debater, turn_payload
...
def test_handle_manual_pairing_uses_configured_name_field_target_name():
    ...
    game_board = GameBoard(TestGameSink())
    game_board.initialize_agents([chooser, bob, cara])
    simulation = TestSimulation([chooser, bob, cara], gameplay_config=GameConfig())
    game = GameMechanicsMixin(game_board, simulation)
    attach_test_runtime(game_board, simulation, game)
